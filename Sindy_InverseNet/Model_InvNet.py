...
def initialize_functions(hyper_params, psi, g, get_params_from_opt, opt_update):
    ...
    def T(params_all, x, dx, z, z_ref):
        ...
        dx_recon = jnp.dot(psi_z(params_psi, z_opt, z_ref), g(params_g, z_opt, z_ref))
        ...
        dz_loss = hyper_params['eta2'] * jnp.sum((jnp.dot(z_opt_x, dx) - g(params_g, z_opt, z_ref)) ** 2)
...
def get_model_and_optimizer(model_params, hyper_params, rng):
    ...
    return psi, g, opt_update, opt_state, get_params_from_opt, init_params


class InvNet():
    def __init__(self, model_params, hyper_params, rng):
        self.psi, self.g, opt_update, self.opt_state, self.get_params_from_opt, init_params = get_model_and_optimizer(
            model_params, hyper_params, rng)
        self.hyper_params = hyper_params
        self.update, self.T_seperate_vec, self.phi_vec, self.psi_vec, self.phi, self.get_L, self.L_z, self.T, self.L = initialize_functions(
            hyper_params, self.psi, self.g, self.get_params_from_opt, opt_update)
    ...
    def forward(self, i, x, dx, z_ref):
        ...
        # self.psi(params_psi, z[0], z_ref[0])
        self.opt_state = self.update(i, self.opt_state, x, dx, z_ref)

    def evaluate(self, X, dX, z_ref, k, name=None):
        z = jnp.zeros((X.shape[0], self.hyper_params['z_latent']))
        params_all = self.get_params()
        params_psi, params_g = params_all[:self.hyper_params['n_psi']], params_all[self.hyper_params['n_psi']:]
        ...
        L = jnp.mean(L, axis=0)
        z_opt = self.phi_vec(params_psi, X, z, z_ref)
        x_rec = self.psi_vec(params_psi, z_opt, z_ref)  # .reshape(-1, 400)
        dx_rec = T[4]
        regul = T[5]
        images = {'P_I: ': x_rec[:, 0], 'Q_I: ': x_rec[:, 1], 'dP_I: ': dx_rec[:, 0], 'dQ_I: ': dx_rec[:, 1],
                  'z0: ': z_opt[:, 0], 'z1: ': z_opt[:, 1], 'dx_real: ': dX[:, 130], 'dx: ': dx_rec[:, 130], 'L: ': L}
        ...
        #    jnp.mean(T[2])) + '  z_dx: ' + str(jnp.mean(T[3])) + '  L1: ' + str(jnp.mean(T[5])))
        print(str(k) + ':   T_loss: ' + str(jnp.mean(T[0])) + '  T_recon: ' + str(jnp.mean(T[1])) + '  T_dx: ' + str(
            jnp.mean(T[2])) + '  z_dx: ' + str(jnp.mean(T[3])) + '  Regul: ' + str(jnp.mean(regul)))

    def predict_45s(self, x, z_ref):
        z0 = jnp.zeros((self.hyper_params['z_latent'],))
        params_all = self.get_params()
        params_psi, params_g = params_all[:self.hyper_params['n_psi']], params_all[self.hyper_params['n_psi']:]
        z_start = self.phi(params_psi, x[0], z0, z_ref[0])
        z = z_start
        z_pred = np.zeros((len(x), self.hyper_params['z_latent']))
        z_pred[0, :] = z_start
        for i, z_ref_next in enumerate(z_ref[1:]):
            dz = self.g(params_g, z, z_ref_next)
            z = z + dz
            z_pred[i, :] = z
        x_pred = self.psi_vec(params_psi, z_pred, z_ref)
        plt.plot(x_pred[:, 0], label='P_I_pred')
        plt.plot(x_pred[:, 1], label='Q_I_pred')
        plt.legend()
        plt.show()
